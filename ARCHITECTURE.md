...
- **Single Backend Ingestion Stream:** The backend maintains a single long-lived Server-Sent Events (`Accept: text/event-stream`) connection to Horizon's `/ledgers` endpoint.
  ...
  - **Only paging tokens are ever used as cursors** ([#84](https://github.com/solaawojobi00-bit/netpulse-xlm/issues/84)). The initial cursor is the paging token of the newest ledger the warm-up fetch returned, and the loop advances it from each record's `paging_token`. Seeding a raw ledger `sequence` — a different kind of identifier, which Horizon reinterprets — is what caused a freshly started backend to ingest ledgers from months earlier alongside current ones. A record arriving without a token leaves the cursor on the last good one rather than falling back to a sequence.
  - A fresh start hands off from the warm-up rather than from `"now"` ([#109](https://github.com/solaawojobi00-bit/netpulse-xlm/issues/109)). Streaming from `"now"` meant the ledgers that closed between the warm-up returning and the SSE connection opening were delivered by neither — one or two per process start per network, missing from SQLite and so undercounted in that day's rollup, which [#87](https://github.com/solaawojobi00-bit/netpulse-xlm/issues/87) keeps indefinitely after pruning the raw rows behind it, making the undercount permanent. Seeding the warm-up page's newest paging token closes that handoff: a token taken from `order=desc` is accepted by the `order=asc` stream and resumes at exactly the next ledger, excluding the cursor ledger itself, so the seam neither duplicates nor skips. `"now"` remains the fallback when no token is available — the warm-up failed or returned nothing, or Horizon omitted the token — which keeps the old behaviour, gap included, rather than refusing to stream.
  - Resumption is still not guaranteed gap-free in general: a reconnect resumes from the last token the loop actually saw, so a record dropped before its token was read is not re-requested. Ledger rows are keyed on `(network, sequence)` with `INSERT OR REPLACE`, so redelivery after a reconnect is idempotent in SQLite.
...
Backend ingestion (persistent SSE stream + ~6s interval for the polled endpoints)
        ...
        │    a fresh start seeds the cursor from the warm-up page's
        │    newest token, so the handoff drops no ledgers
