...
describe("HistoryView", () => {
  it("renders a sensible empty state when points array is empty", () => {
    ...
    expect(
      screen.getByText(/Historical trend data is accumulating/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/5m resolution · 7-day retention/i),
    ).toBeInTheDocument();
  });
  ...
  it("renders chart headings and subtitle when points are provided", () => {
    ...
    expect(
      screen.getByText("12h Ledger close time (avg seconds)"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("12h Network congestion (capacity usage %)"),
    ).toBeInTheDocument();
  });
  ...
  it("shows a loading state before the first fetch resolves", () => {
    ...
    expect(
      screen.queryByText(/Historical trend data is accumulating/i),
  ...
  it("surfaces a failed history fetch instead of looking empty", () => {
    ...
    expect(
      screen.getByText(/Could not load historical trends/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/Historical trend data is accumulating/i),
    ).not.toBeInTheDocument();
  ...
  it("states only once that history failed, not once per chart", () => {
    ...
    expect(
      screen.getAllByText(/Could not load historical trends/i),
    ).toHaveLength(1);
  });
  ...
  it("keeps showing existing history when a later refresh fails", () => {
    ...
    render(
      <HistoryView points={mockPoints} range="24h" error="Failed to fetch" />,
    );

    expect(
      screen.getByText("24h Ledger close time (avg seconds)"),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/Could not load historical trends/i),
    ).not.toBeInTheDocument();
  });
});
